...
def mesh_reconstruction(config):
  ...
  for num_iter in range(config['max_iter']):
    ...
    if np.argmax(pred) == config['target_label'] and np.argmax(pred) >= config['pred_close_enough_to_target']:
      skipped_iters += 1
      if skipped_iters < 10:
    ...
    source_pred_diff = abs(source_pred_brfore_attack - source_pred_after_attack)
    target_pred_diff = abs(target_pred_brfore_attack - target_pred_after_attack)

    if source_pred_diff > config['max_label_diff'] or target_pred_diff > config['max_label_diff']:
        ratio = config['max_label_diff'] / max(source_pred_diff, target_pred_diff)
