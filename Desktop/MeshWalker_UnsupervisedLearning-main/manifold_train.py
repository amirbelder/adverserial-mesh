...
def train_val(params):
  ...
  def train_step(model_ftrs_, labels_, one_label_per_model):
    ...
    with tf.GradientTape() as tape:
      if one_label_per_model:
        ...
        #Amir - will later change to alpha = 0
        if params.train_loss == ['manifold_cros_entr']:
            predictions = dnn_model(model_ftrs, alpha, shift_size)
        else:
        ...
        if params.train_loss == ['manifold_cros_entr']:
            predictions = dnn_model(model_ftrs, alpha, shift_size)[:, skip:]
        else:
  ...
  def test_step(model_ftrs_, labels_, one_label_per_model):
    ...
    if one_label_per_model:
      ...
      skip = params.min_seq_len
      if params.train_loss == ['manifold_cros_entr']:
          predictions = dnn_model(model_ftrs, alpha, shift_size, training=False)[:, skip:]
      else:
