...
def calc_segmentaion_retults(logdir, model_fn, pathname_expansion, output_fn):
...
def make_segmentaion_figures(dataset, shape_id=1):
  ...
  if 0:
    ...
    logdir = '/media/alonlahav/0.5T/runs/runs_test/0017-29.06.2020..08.15__human_seg-4k/'
    model_fn = 'learned_model2keep__00140008.keras'
  elif 1:
    logdir = os.path.expanduser('~') + '/mesh_walker/runs_pretrained/0001-09.08.2020..15.19__human_seg-6k_unit_bal_norm-4upload/'
    pathname_expansion = os.path.expanduser('~') + '/mesh_walker/datasets_processed/' + 'sig17_seg_benchmark-6k/test__shrec_' + \
                         str(shape_id) + '_simplified_to_6000.npz'
    model_fn = 'learned_model2keep__00040007.keras'

  my_res_fn = logdir + '/seg_res__shape_' + str(shape_id)
  if 1:
    mesh_data = calc_segmentaion_retults(logdir, model_fn, pathname_expansion, output_fn=my_res_fn)
  else:
