...
def get_model_by_name(name):
  ...
  if 'labels_fuzzy' in mesh_data.keys():
    ...
    if fn.find('/coseg/') != -1:
      ...
      part = name.split(':')[0]
      seseg_fn = os.path.expanduser('~') + '/datasets/coseg/' + part + '/sseg/' + n + '.seseg'
    if fn.find('test_shrec__') != -1:
      n = fn.split('/')[-1].split('__')[-1].split('_')[0]
      seseg_fn = os.path.expanduser('~') + '/mesh_walker/datasets/human_seg/sseg/shrec__' + n + '.seseg'
  ...
  if 1 and 'shrec' in fn:
    ...
    if idx == '12':
      ...
      o_fn = idx + '.off'
    o_mesh = trimesh.load_mesh(os.path.expanduser('~') + '/mesh_walker/datasets/sig17_seg_benchmark/meshes/test/shrec/' + o_fn, process=False)
    if idx == '15' and USED_FIXED_LABEL_FOR_SHREC15_HUMAN:
      o_l_fn = 'shrec_' + idx + '_full.txt.fixed.txt'
    else:
      o_l_fn = 'shrec_' + idx + '_full.txt'
    o_face_labels = np.loadtxt(os.path.expanduser('~') + '/mesh_walker/datasets/sig17_seg_benchmark/segs/test/shrec/' + o_l_fn)
...
def calc_final_accuracy(models, print_details, ignore_label=None):
  ...
  for model_name, model in models.items():
    ...
    # ignore labels if needed
    if ignore_label is not None:
      indices = np.nonzero(1 - (model['labels'] == ignore_label))
      best_pred = best_pred[indices]
      model['labels'] = model['labels'][indices]
      model['area_vertices'] = model['area_vertices'][indices]
      model['pred'] = model['pred'][indices]
      model['v_indices'] = indices
...
def IoU_from_confusions(confusions):
  ''' Taken from KPConv github - https://github.com/HuguesTHOMAS/KPConv/blob/master/utils/metrics.py'''
  """
...
def calc_final_miou(models, print_details):
  # calculate confusion matrices and than IoU from it - same as KPCONV.
  # === stacking all predictions together over n_samples
  all_labels = []
  all_preds = []
  for model_name, model in models.items():
    best_pred = np.argmax(model['pred'], axis=-1)
  ...
  C_mat = confusion_matrix(np.concatenate(all_labels, axis=0),
                           np.concatenate(all_preds, axis=0))
  ACC_verify = 100 * np.sum(np.diag(C_mat)) / (np.sum(C_mat) + 1e-6)
  print('Accuracy calcuated from confusion matrix -  {:2.3f}'.format(ACC_verify))
  IoU = IoU_from_confusions(C_mat)
  print('mIoU -  {:2.3f}'.format(np.mean(IoU)))
  return np.mean(IoU)

  # TODO: check if its done the same way as point cloud- since they INTERPOLATE to test points from selected points (downsampled points)
...
def calc_accuracy_test(dataset_folder=False, logdir=None, dnn_model=None, params=None, verbose_level=2,
  ...
  if not 'scannet' in dataset_folder:
  ...
  # for iter in tqdm(range(1), disable=print_details):
    i_m = 0
    for name_, model_ftrs_, labels_ in test_dataset:
      ...
      for g in range(walks_f):
        predictions_ = tf.nn.softmax(dnn_model(ftrs[g * strd:(g + 1) * strd], training=False), axis=-1).numpy()[:, skip:]
      ...
      all_seq = all_seq[0, :, skip:].reshape(-1).astype(np.int32)
  ...
  ignore_label = params.ignore_label if hasattr(params, 'ignore_label') else None
  e_acc_after_postproc, v_acc_after_postproc, f_acc_after_postproc = calc_final_accuracy(models, print_details, ignore_label)
  miou = calc_final_miou(models, print_details)
  ...
  return {'mIoU': miou,
          'face_accuracy': f_acc_after_postproc,
          'vertices_accuracy':v_acc_after_postproc,
          'edges_accuracy': e_acc_after_postproc}, dnn_model


def calc_accuracy_test_scannet(dataset_folder=False, logdir=None, dnn_model=None, params=None, verbose_level=2,
                       n_iters=None, dump_model=False, min_max_faces2use=[0, np.inf], model_fn=None,
                       n_target_faces=None, full_coverage=False,
                       n_walks_per_model=32, seq_len=None, params2overide={}, data_augmentation={}):
  '''
  Since scannet is a huge dataset, we need to calc accuracy in slices, else it consumes all RAM and fails
  Need to: create .txt with predictions - one line per vertex in every area name
  ...
  skip = int(params.seq_len * 0.5)
  test_accuracy = tf.keras.metrics.SparseCategoricalAccuracy(name='test_accuracy')
  n_walks = 0  # TODO: change this loop, instead of n_iters get dataloader to handle mesh coverage (since random walks - we need to walk all vertices at least X times...)
  # for iter in tqdm(range(1), disable=print_details):
  i_m = 0
  test_areas = glob.glob(pathname_expansion)
  if not full_coverage:
    test_areas = test_areas[:10]
  for area in test_areas:
    '''
    Calculating accuracy for each area alone, then combine results together
    '''
    models = {}
    test_dataset, n_items = dataset.tf_scannet_dataset(params, area, shuffle_size=0, full_coverage=full_coverage)
    if dnn_model is None:
      dnn_model = rnn_model.RnnWalkNet(params, params.n_classes, params.net_input_dim - 1, model_fn,
                                       model_must_be_load=True,
                                       dump_model_visualization=False)
    for _ in range(8):   # repeat to get more preds per vertex
      for name_, model_ftrs_, labels_ in test_dataset:
          name = name_.numpy()[0].decode()
          i_m += 1
          assert name_.shape[0] == 1
          ...
          model_ftrs = model_ftrs_[:, :, :, :-1] * scale_augmentation
          all_seq = model_ftrs_[:, :, :, -1].numpy()
          total_walks_per_area = model_ftrs.shape[1]
          if name not in models.keys():
            models[name] = get_model_by_name(name)
            models[name]['pred'] = np.zeros((models[name]['vertices'].shape[0], params.n_classes))
          ...
          labels = tf.reshape(labels_, (-1, sp[-2]))
          batch_size = 32
          strd = math.ceil(total_walks_per_area / batch_size)
          predictions = None
          for g in range(strd):
            predictions_ = tf.nn.softmax(dnn_model(ftrs[g * batch_size:(g + 1) * batch_size], training=False), axis=-1).numpy()[:,
                           skip:]
            if predictions is None:
              predictions = np.zeros((total_walks_per_area, predictions_.shape[1], predictions_.shape[2]))
            predictions[g * batch_size:(g + 1) * batch_size] = predictions_
          all_seq = all_seq[0, :, skip:].reshape(-1).astype(np.int32)
          predictions4vertex = predictions.reshape((-1, predictions.shape[-1]))
          labels = labels[:, skip:]
          for w_step in range(all_seq.size):
    ...
    assert models[name]['pred'].shape[0] == models[name]['vertices'].shape[0]
    # Calculate final prediction per vertex for area
    postprocess_vertex_predictions(models)
    ignore_label = params.ignore_label if hasattr(params, 'ignore_label') else None
    ...
    # Save .txt with prediction per vertex, save .txt with label per vertex (if exists)
    valid_indices = models[name]['v_indices']
    all_v_preds = np.zeros(models[name]['vertices'].shape[0])
    all_v_preds[valid_indices[0]] = np.argmax(models[name]['pred'], axis=-1)
    all_v_labels = -1 * np.ones(models[name]['vertices'].shape[0])
    all_v_labels[valid_indices] = models[name]['labels']
    pred_save_path = os.path.join(params.logdir, 'eval_scannet', 'pred', name.split('/')[-1].replace('.npz', '.txt'))
    if not os.path.exists(os.path.dirname(pred_save_path)):
      os.makedirs(os.path.dirname(pred_save_path))
    with open(pred_save_path, 'w') as f:
      for l in all_v_preds:
        f.write(str(int(l)) + '\n')
    gt_save_path = pred_save_path.replace('pred', 'gt')
    if not os.path.exists(os.path.dirname(gt_save_path)):
      os.makedirs(os.path.dirname(gt_save_path))
    with open(gt_save_path, 'w') as f:
      for l in all_v_labels:
        f.write(str(int(l)) + '\n')

  # TODO: calculate final acc\mio from ALL model .txt files
  print('TODO: still need to calculate final accuracy on all val/test areas - from saved .txt files')
  # need to get MioU, per class IoU, mAcc, overall acc
  from scannet_preprocess.evaluation.evaluate_semantic_label import scannet_v2_eval
  pred_files = glob.glob(os.path.join(params.logdir, 'eval_scannet', 'pred', '*.txt'))
  pred_files.sort()
  gt_files = glob.glob(os.path.join(params.logdir, 'eval_scannet', 'gt', '*.txt'))
  gt_files.sort()
  out_path = os.path.join(params.logdir, 'eval_scannet', 'semantic_label_evaluation.txt')
  scannet_v2_eval(pred_files, gt_files, out_path)
  ...
  if dump_model:
    for colorize_only_bad_pred in [0, 1]:
      for name, model in tqdm(models.items()):
        if 0:
          utils.visualize_model(model['vertices'], model['faces'], v_size=1, show_vertices=0,
                                face_colors=model['face_labels'], show_edges=0)
          utils.visualize_model(model['vertices'], model['faces'], v_size=1, show_vertices=0,
                                face_colors=model['f_pred'], show_edges=0)
          utils.visualize_model(model['vertices'], model['faces'], vertex_colors_idx=model['labels'].astype(np.int),
                                point_size=5, show_edges=0)
          utils.visualize_model(model['vertices'], model['faces'], vertex_colors_idx=model['v_pred'].astype(np.int),
                                point_size=5, show_edges=0)
        pred_per_node = model['pred']
        no_pred_idxs = np.where(np.abs(pred_per_node).sum(axis=1) == 0)[0]
        best_per_node = np.argmax(pred_per_node, axis=1)
        ...
        if 'original' in models[name].keys():
          o_model = EasyDict(
            {'vertices': models[name]['original']['o_vertices'], 'faces': models[name]['original']['o_faces']})
          o_f_colors = np.zeros((o_model['faces'].shape[0], 3))
          for i, label in enumerate(models[name]['original']['o_face_labels']):
            pred = models[name]['original']['o_face_preds'][i]
            if not colorize_only_bad_pred or pred != label:
...
if __name__ == '__main__':
  ...
  n_target_faces = None
  logdir = os.path.expanduser('~') + '/mesh_walker/'
  ds = 'scannet_v2_4cm_crops'
  logdir += 'runs_aug_360_must/0091-08.11.2020..08.31__scannet_v2'
  model_fn = glob.glob(logdir + '/learned_model2keep*.keras')
  model_fn.sort()
  model_fn = model_fn[-1]
  # model_fn = None # logdir + 'learned_model2keep__00010010.keras'

  if 1: # Run on all test meshes
    dataset2use = os.path.expanduser('~') + '/mesh_walker/datasets/' + ds + '/*val*.npz'
  else: # Run on one mesh, only for visualize
    verbose_level = 0
    mesh_id = '183' # 180 / 100 / 008 / 016 / 018
    dataset2use = '/home/alonlahav/datasets/MPI-FAUST/test/scans/test_scan_' + mesh_id + '.ply'
    n_target_faces = 4000

  params2overide = EasyDict()
  #params2overide.n_target_vrt_to_norm_walk = 3000
  if 'scannet' in logdir:
    calc_accuracy_test_scannet(logdir=logdir, dataset_folder=dataset2use, n_iters=n_iters, dump_model=True,
