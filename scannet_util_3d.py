...
def load_ids(filename):
  ids = [int(float(x)) for x in open(filename).read().splitlines()]
...
def read_instance_prediction_file(filename, pred_path):
  ...
  for line in lines:
    ...
    if os.path.isabs(parts[0]):
      scannet_util.print_error('invalid instance prediction file. First entry in line must be a relative path')
    ...
    if os.path.commonprefix([mask_file, abs_pred_path]) != abs_pred_path:
      scannet_util.print_error('predicted mask {} in prediction text file {} points outside of prediction path.'.format(mask_file, filename))
...
def interpolate_points(orig_verts, ds_verts, k=3):
  ball_tree = BallTree(ds_verts)
  # gonna be slow...  need k neighbors for each point in orig_verts